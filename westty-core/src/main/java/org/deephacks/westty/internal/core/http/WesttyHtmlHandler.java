...
class WesttyHtmlHandler extends WesttyHttpHandler {
    ...
    public WesttyHtmlHandler(File rootPath, List<WesttyTemplateCompiler> compilers) {
    ...
    private ChannelBuffer compileTemplate(ChannelHandlerContext ctx, String uri)
        ...
        TemplateContext tctx = new TemplateContext(new StringWriter(), new BufferedReader(
                new FileReader(new File(staticRootPath + uri))), uri);
        for (WesttyTemplateCompiler compiler : compilers) {
            if (compiler.process(tctx)) {
                ChannelBuffer buf = ChannelBuffers.wrappedBuffer(tctx.getWriter().toString()
                        .getBytes());
                return buf;
            }
        }
